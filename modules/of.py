"""
WeedHack Mod Injector v6.1 - Auto-downloads Forge/Fabric libraries
"""
import os, sys, shutil, tempfile, zipfile, subprocess, traceback, json
...
# ── Forge version → known installer version mapping ──────────────────────
# These are the "recommended" or stable build numbers for each MC version.
# You can expand this list or make the script scrape the Forge site.
FORGE_VERSIONS = {
    "1.20.1": "47.4.10",
    "1.19.4": "45.3.7",
    "1.19.2": "43.5.9",
...
def get_minecraft_dir():
    ...
    if sys.platform == "win32":
        ...
        base = os.path.join(os.environ.get("HOME", ""), ".minecraft")
    if os.path.exists(base):
        return base
    return base  # might not exist yet
...
def find_javac():
    ...
    if java_home:
        for name in ("javac", "javac.exe"):
            p = os.path.join(java_home, "bin", name)
            if os.path.exists(p):
                return p
